...
=========================
Generates data/cities.csv from authoritative third-party source datasets.

Usage (run from the data/ directory):
    python build.py

Source files live in data/sources/. All of them are generated by
sources/download.py. Every score in cities.csv comes from a real source;
when a source has no data for a city the cell is left empty and the app
skips that dimension for that city (showing a coverage badge).

Adding a new city
-----------------
1. Add a row to sources/city_meta.csv
2. Add the city's name variants to sources/city_mappings.csv
3. Run: python sources/download.py   (fetches data for the new city)
...
SOURCES = {
    ...
    'airQuality': {
        'name':  'WHO Ambient Air Quality Database (city PM2.5 annual means)',
        'url':   'https://www.who.int/data/gho/data/themes/air-pollution/who-air-quality-database',
        'file':  'sources/who_air.csv',
        'level': 'city',
        'year':  2024,
        'note':  (
            'Generated by sources/download.py from the WHO database (V6.1): '
            'each city is geo-matched to the nearest WHO settlement. '
            'Expected columns: "city_id", "PM2.5" (annual mean µg/m3). '
    ...
    'govProgressiveness': {
        ...
        'file':  'sources/freedom_house.csv',
        'level': 'country',
        'year':  2026,
        'note':  (
            'Generated by sources/download.py from the FIW country scores page. '
            'Aggregate score 0-100 (higher = more free). Used directly.'
        ),
    },
    'healthcareQuality': {
        ...
        'file':  'sources/who_uhc.csv',
        'level': 'country',
        'year':  2024,
        'note':  (
            'Generated by sources/download.py from the WHO GHO API '
            '(UHC_INDEX_REPORTED, latest year per country). Index 0-100. Used directly.'
        ),
    },
    'educationQuality': {
        'name':  'World Bank Harmonized Learning Outcomes',
        'url':   'https://data.worldbank.org/indicator/HD.HCI.HLOS',
        'file':  'sources/wb_hlo.csv',
        'level': 'country',
        'year':  2020,
        'note':  (
            'Generated by sources/download.py. Harmonized test scores (~300-625) '
            'rescaled to 0-100 at download time via (score - 300) / 300 * 100.'
        ),
    },
    'economicOutlook': {
        'name':  'World Bank real GDP growth (3-year average)',
        'url':   'https://data.worldbank.org/indicator/NY.GDP.MKTP.KD.ZG',
        'file':  'sources/wb_gdp.csv',
        'level': 'country',
        'year':  2025,
        'note':  (
            'Generated by sources/download.py. 3-year average real GDP growth '
            'mapped to 0-100 at download time via (avg + 2) / 8 * 100.'
        ),
    },
    'fossilFuelReliance': {
        'name':  'Our World in Data / Ember — fossil share of electricity',
        'url':   'https://ourworldindata.org/grapher/share-electricity-fossil-fuels',
        'file':  'sources/owid_fossil.csv',
        'level': 'country',
        'year':  2024,
        'note':  (
            'Generated by sources/download.py. Score = 100 - fossil share of '
            'electricity generation (computed at download time).'
        ),
    },
    'visaEase': {
...
def load_ti_cpi(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — corruption will be skipped where absent")
        return {}
...
def load_rsf(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — pressFreedom will be skipped where absent")
        return {}
...
def load_sipri(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — militarySpending will be skipped where absent")
        return {}
...
def load_ookla(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — internetQuality will be skipped where absent")
        return {}
...
def load_ef_epi(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — languageAccess will be skipped where absent")
        return {}
...
def load_henley(city_meta, country_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — visaEase will be skipped where absent")
        return {}
...
def load_country_score(dim, city_meta, label):
    """
    Generic loader for country-level sources whose files already contain
    our canonical country names and final 0-100 scores (columns:
    "Country", "Score"). download.py generates these.
    """
    path = src(SOURCES[dim]['file'])
    if not os.path.exists(path):
        warn(f"Missing {path} — {dim} will be skipped where absent")
        return {}
    ...
    for row in rows:
        try:
            scores[row['Country'].strip().lower()] = int(float(row['Score']))
        except (ValueError, KeyError):
            pass

    result = {}
    for city in city_meta:
        val = scores.get(city['country'].strip().lower())
        if val is not None:
            result[city['id']] = val
    info(f"{dim} ({label}): updated {len(result)}/{len(city_meta)} cities")
    return result
...
def load_numbeo_col(city_meta, city_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — costOfLiving will be skipped where absent")
        return {}
...
def load_numbeo_property(city_meta, city_maps):
    ...
    The upper bound is capped at 40: a few cities report freak ratios
    (Lagos >100) that would otherwise compress the whole scale; anything
    at 40+ years-of-income is equally unaffordable and scores 0.
    """
    path = src(SOURCES['affordability']['file'])
    if not os.path.exists(path):
        warn(f"Missing {path} — affordability will be skipped where absent")
        return {}
    ...
    lo, hi = min(raw_values), min(max(raw_values), 40)
    result = {}
    updated = 0
    for city in city_meta:
        cid = city['id']
        ...
        val = src_lookup.get((numbeo_city, numbeo_country))
        if val is not None:
            result[cid] = normalize_invert(min(val, hi), lo, hi)
            updated += 1
    info(f"affordability (Numbeo Property): updated {updated}/{len(city_meta)} cities (raw range {lo:.1f}-{hi:.1f} capped)")
    return result
...
def load_numbeo_crime(city_meta, city_maps):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — safety will be skipped where absent")
        return {}
...
def load_who_air(city_meta):
    """
    WHO Ambient Air Quality Database PM2.5 annual mean -> airQuality.
    who_air.csv is keyed by city_id (download.py geo-matches each city
    to a WHO settlement), so no name-matching is needed here.
    Transform: clamp((50 - pm25) / 50 * 100, 0, 100)
    WHO guideline = 5 µg/m3. At 50 µg/m3 score = 0.
    """
    path = src(SOURCES['airQuality']['file'])
    if not os.path.exists(path):
        warn(f"Missing {path} — airQuality will be skipped where absent")
        return {}

    rows = read_csv(path)
    pm_col = next((c for c in rows[0] if 'pm2' in c.lower() or 'concentration' in c.lower()), None)
    if 'city_id' not in rows[0] or not pm_col:
        warn(f"WHO air: missing expected columns. Columns: {list(rows[0].keys())}")
        return {}

    result = {}
    for row in rows:
        try:
            result[row['city_id']] = clamp(round((50 - float(row[pm_col])) / 50 * 100))
        except (ValueError, KeyError):
            pass
    info(f"airQuality (WHO): updated {len(result)}/{len(city_meta)} cities")
    return result
...
def load_climate_data(city_meta):
    ...
    if not os.path.exists(path):
        warn(f"Missing {path} — sunshine/humidity/rainfall/winterWarmth will be skipped where absent")
        return {}
...
ALL_DIMS = [
    'corruption', 'pressFreedom', 'costOfLiving', 'affordability',
    'govProgressiveness', 'militarySpending', 'economicOutlook',
    'fossilFuelReliance', 'safety', 'healthcareQuality', 'educationQuality',
    'internetQuality', 'airQuality', 'languageAccess', 'visaEase',
...
def build():
    ...
    overrides = {
        ...
        'airQuality':      load_who_air(city_meta),
        ...
        'govProgressiveness': load_country_score('govProgressiveness', city_meta, 'Freedom House'),
        'healthcareQuality':  load_country_score('healthcareQuality', city_meta, 'WHO UHC'),
        'educationQuality':   load_country_score('educationQuality', city_meta, 'WB HLO'),
        'economicOutlook':    load_country_score('economicOutlook', city_meta, 'WB GDP growth'),
        'fossilFuelReliance': load_country_score('fossilFuelReliance', city_meta, 'OWID/Ember'),
    }
    ...
    for city in city_meta:
        ...
        # Dimension columns: real source data or empty (the app skips
        # empty dimensions for that city — no fabricated values)
        filled = 0
        for dim in ALL_DIMS:
    ...
    total_cells = len(output_rows) * len(ALL_DIMS)
    print(f"Wrote {len(output_rows)} cities -> cities.csv")
    print(f"Coverage: {filled_cells}/{total_cells} scores "
          f"({filled_cells / total_cells * 100:.1f}%), "
          f"{full_coverage}/{len(output_rows)} cities fully covered")
    print()
